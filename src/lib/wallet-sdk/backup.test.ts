...
describe('wallet-sdk/backup', () => {
  ...
  describe('encryptSeedPhrase', () => {
    ...
    it('should return the correct filename format', async () => {
      ...
      expect(result.filename).toBe(`wallet_tempesttouchportal_${testWalletId}_seedphrase.txt.gpg`);
    });
    ...
    it('should produce different filenames for different wallet IDs', async () => {
      ...
      expect(result1.filename).toBe('wallet_tempesttouchportal_wallet-1_seedphrase.txt.gpg');
      expect(result2.filename).toBe('wallet_tempesttouchportal_wallet-2_seedphrase.txt.gpg');
    });
  });
