...
export function generateMnemonic(): string {
  // Using simple word list for demo (should use proper BIP39 wordlist)
  const words = [
    ...
    'cute', 'cycle', 'damage', 'damp', 'dance', 'danger', 'daring', 'dash',
    ...
    'name', 'napkin', 'narrow', 'nasty', 'nation', 'nature', 'near', 'neat',
    ...
    'oval', 'oven', 'over', 'own', 'owner', 'oxygen', 'oyster', 'ozone', 'pact',
    ...
    'represent', 'reproduce', 'public', 'require', 'rescue', 'resemble', 'resist',
    ...
    'rocket', 'romance', 'roof', 'rookie', 'room', 'root', 'rose', 'rotate',
    'rotten', 'rough', 'round', 'route', 'royal', 'rubber', 'rude', 'rug',
    ...
    'struggle', 'student', 'stuff', 'stumble', 'stupid', 'style', 'subject', 'submit',
    ...
    'swamp', 'swap', 'swarm', 'swear', 'sweat', 'sweep', 'sweet', 'swift',
    ...
    'title', 'toast', 'tobacco', 'toddler', 'toe', 'together', 'toilet', 'token',
  ...
  for (let i = 0; i < 12; i++) {
    // Create a 11-bit index from the entropy
    const byteIndex = Math.floor(i * 11 / 8);
    const bitOffset = (i * 11) % 8;
    
    let value = entropy[byteIndex] >> (3 - bitOffset);
    if (bitOffset > 5) {
      value |= entropy[byteIndex + 1] << (8 - bitOffset);
    }
    indices.push(value % 2048);
  }
