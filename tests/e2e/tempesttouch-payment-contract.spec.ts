...
const businessId = 'business-tempesttouch-contract';
const merchantWalletAddress = '0x1111111111111111111111111111111111111111';
const tempesttouchMiddlemanAddress = '0x2222222222222222222222222222222222222222';
const paymentId = 'payment-tempesttouch-contract';

async function seedTempestTouchCreateRoutes(page: Page) {
  ...
  await page.route('**/api/businesses', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        success: true,
        businesses: [{ id: businessId, name: 'Tempest Touch Contract Business' }],
      }),
    });
  });
...
test('Tempest Touch create payment uses a middleman payment address, not the merchant wallet', async ({ page }) => {
  await seedTempestTouchCreateRoutes(page);
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        payment: {
          ...
          payment_address: tempesttouchMiddlemanAddress,
  ...
  await expect(page.getByText(tempesttouchMiddlemanAddress)).toBeVisible();
