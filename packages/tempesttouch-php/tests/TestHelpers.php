...
if ( ! function_exists('tempesttouch_test_run') ) {
    $GLOBALS['__tempesttouch_test_count']  = 0;
    $GLOBALS['__tempesttouch_test_failed'] = 0;

    function tempesttouch_test_run(string $label, callable $fn): void
    {
        $GLOBALS['__tempesttouch_test_count']++;
        try {
            $fn();
            echo "[pass] $label\n";
        } catch (\Throwable $e) {
            $GLOBALS['__tempesttouch_test_failed']++;
            echo "[FAIL] $label — " . $e->getMessage() . "\n";
            echo "       at " . $e->getFile() . ":" . $e->getLine() . "\n";
        }
    }

    function tempesttouch_test_summary(): void
    {
        $count  = (int) $GLOBALS['__tempesttouch_test_count'];
        $failed = (int) $GLOBALS['__tempesttouch_test_failed'];
        $passed = $count - $failed;
        echo "\n$passed/$count passed";
        if ($failed > 0) {
            echo " — $failed FAILED\n";
    ...
    function tempesttouch_assert_true($cond, string $why = ''): void
    {
        if (!$cond) {
    ...
    function tempesttouch_assert_equals($expected, $actual, string $why = ''): void
    {
        if ($expected !== $actual) {
    ...
    function tempesttouch_assert_contains(string $needle, string $haystack, string $why = ''): void
    {
    ...
    function tempesttouch_assert_throws(callable $fn, string $expectedExceptionClass, string $why = ''): \Throwable
    {
        try {
            $fn();
        } catch (\Throwable $e) {
