 ...
 * @param {TempestTouchClient} [params.client] - Existing client
...
export async function getTokens({ apiKey, client, businessId, activeOnly } = {}) {
  ...
  return tempesttouch.getTokens({ businessId, activeOnly });
}
...
export async function getSupportedCoins({ apiKey, client, businessId, activeOnly } = {}) {
  const tempesttouch = client || new TempestTouchClient({ apiKey });
  return tempesttouch.getSupportedCoins({ businessId, activeOnly });
}
