...
/**
 * Tempest Touch CLI
 * Command-line interface for Tempest Touch cryptocurrency payments
...
const HOME_DIRECTORY = process.env.TEMPESTTOUCH_HOME || process.env.HOME || homedir();
const CONFIG_FILE = join(HOME_DIRECTORY, '.tempesttouch.json');
const DEFAULT_WALLET_FILE = join(HOME_DIRECTORY, '.tempesttouch-wallet.gpg');
...
function getApiKey() {
  const config = loadConfig();
  return process.env.TEMPESTTOUCH_API_KEY || config.apiKey;
}
...
function getBaseUrl() {
  const config = loadConfig();
  return process.env.TEMPESTTOUCH_BASE_URL || config.baseUrl || 'https://tempesttouch.com/api';
}
...
function createClient() {
  ...
  if (!apiKey) {
    print.error('API key not configured. Run: tempesttouch config set-key <api-key>');
    process.exit(1);
  }
  return new TempestTouchClient({ apiKey, baseUrl: getBaseUrl() });
}
...
function createUnauthenticatedClient() {
  return new TempestTouchClient({ baseUrl: getBaseUrl() });
}
...
function showHelp() {
  console.log(`
${colors.bright}Tempest Touch CLI${colors.reset} v${VERSION}

${colors.cyan}Usage:${colors.reset}
  tempesttouch <command> [options]
...
${colors.cyan}Wallet Options:${colors.reset}
  ...
  --wallet-file <path>    Custom wallet file (default: ~/.tempesttouch-wallet.gpg)
...
${colors.cyan}Examples:${colors.reset}
  ...
  tempesttouch wallet create --words 12

  # Import existing wallet
  tempesttouch wallet import "word1 word2 ... word12"

  # Get wallet balance (auto-decrypts)
  tempesttouch wallet balance

  # Send transaction (auto-decrypts for signing)
  tempesttouch wallet send --chain ETH --to 0x123... --amount 0.1
  
  # Send transaction with fiat amount
  tempesttouch wallet send --chain SOL --to abc123... --amount-fiat 10 --fiat USD

  # Create escrow with crypto amount
  tempesttouch escrow create --chain SOL --amount 0.5 --depositor abc... --beneficiary def...
  
  # Create escrow with fiat amount
  tempesttouch escrow create --chain SOL --amount-fiat 50 --fiat USD --depositor abc... --beneficiary def...
  
  # Authenticate with escrow token
  tempesttouch escrow auth escr_123 --token rel_abc456

  # Swap BTC to ETH
  tempesttouch swap quote --from BTC --to ETH --amount 0.1
  tempesttouch swap create --from BTC --to ETH --amount 0.1 --settle 0x...

  # Connect Stripe to a business (country is locked at Stripe account creation)
  tempesttouch card connect onboard <business-id> --country CA --email merchant@example.com

  # List payment tokens for a business
  tempesttouch tokens list --business-id <business-id> --active-only

${colors.cyan}Environment Variables:${colors.reset}
  TEMPESTTOUCH_API_KEY         API key (overrides config)
...
async function saveEncryptedWallet(mnemonic, walletId, password, walletFile) {
  ...
  const tmpFile = join(tmpdir(), `tempesttouch-wallet-${Date.now()}.json`);
  const passFile = join(tmpdir(), `tempesttouch-pass-${Date.now()}`);
  ...
  switch (subcommand) {
    case 'create': {
      ...
      if (!businessId || !amount || !blockchain) {
        ...
        print.info('Example: tempesttouch payment create --business-id biz_123 --amount 100 --blockchain BTC');
        return;
      }
  ...
  switch (subcommand) {
    ...
    case 'import': {
      ...
      if (!mnemonic) {
        ...
        print.info('Usage: tempesttouch wallet import "word1 word2 ... word12"');
        return;
      }
    ...
    case 'unlock': {
      if (!hasEncryptedWallet(flags)) {
        ...
        print.info('Create a wallet with: tempesttouch wallet create');
        return;
      }
    ...
    case 'info': {
      if (!config.walletId && !hasEncryptedWallet(flags)) {
        print.error('No wallet configured. Run: tempesttouch wallet create');
        return;
      }
    ...
    case 'derive': {
      ...
      if (!chain) {
        ...
        print.info('Usage: tempesttouch wallet derive ETH --index 0');
        return;
      }
    ...
    case 'send': {
      ...
      if (!chain || !to || (!amount && !amountFiat)) {
        ...
        print.info('Usage: tempesttouch wallet send --chain ETH --to 0x123... --amount 0.1');
        print.info('   or: tempesttouch wallet send --chain SOL --to abc123... --amount-fiat 10 --fiat USD');
        return;
      }
    ...
    case 'backup': {
      ...
      const outputPath = flags.output || `tempesttouch-wallet-backup-${Date.now()}.gpg`;
  ...
  switch (subcommand) {
    ...
    case 'quote': {
      ...
      if (!from || !to || !amount) {
        ...
        print.info('Example: tempesttouch swap quote --from BTC --to ETH --amount 0.1');
        return;
      }
    ...
    case 'create': {
      ...
      if (!from || !to || !amount) {
        print.error('Required: --from, --to, --amount');
        print.info('Example: tempesttouch swap create --from BTC --to ETH --amount 0.1 --settle 0x...');
        return;
      }
    ...
    case 'status': {
      ...
      if (!swapId) {
        ...
        print.info('Usage: tempesttouch swap status <swap-id>');
        return;
      }
...
async function handleEscrow(subcommand, args, flags) {
  ...
  switch (subcommand) {
    case 'create': {
      ...
      print.info(`  Manage: tempesttouch escrow auth ${escrow.id} --token <token>`);
  ...
  switch (subcommand) {
    case 'register': {
      ...
      if (!email || !password) {
        ...
        print.info('Example: tempesttouch auth register --email user@example.com --password mysecret --name "My Company"');
        return;
      }
    ...
    case 'login': {
      ...
      if (!email || !password) {
        print.error('Required: --email <email> --password <password>');
        print.info('Example: tempesttouch auth login --email user@example.com --password mysecret');
        return;
      }
    ...
    case 'me': {
      try {
        ...
        if (!jwtToken) {
          print.error('Not authenticated. Run: tempesttouch auth login');
          return;
        }
        
        // Create client with JWT token instead of API key
        const client = new TempestTouchClient({ apiKey: jwtToken, baseUrl: getBaseUrl() });
        ...
        if (error.message.includes('401') || error.message.includes('403')) {
          print.error('Authentication failed. Please run: tempesttouch auth login');
        } else {
          print.error(error.message);
        }
      }
  ...
  switch (subcommand) {
    case 'submit': {
      ...
      if (!receiptArg) {
        ...
        print.info('Example: tempesttouch reputation submit --receipt receipt.json');
        print.info('Example: tempesttouch reputation submit --receipt \'{"receipt_id":"..."}\'');
        return;
      }
    ...
    case 'query': {
      ...
      if (!agentDid) {
        ...
        print.info('Usage: tempesttouch reputation query <agent-did> [--window 30d|90d|all]');
        return;
      }
    ...
    case 'credential': {
      ...
      if (!credentialId) {
        ...
        print.info('Usage: tempesttouch reputation credential <credential-id>');
        return;
      }
    ...
    case 'verify': {
      ...
      if (!credentialId) {
        print.error('Credential ID required');
        print.info('Usage: tempesttouch reputation verify <credential-id>');
        return;
      }
    ...
    case 'did': {
      ...
      if (didSubcommand === 'claim') {
        ...
        if (!didValue || !publicKey || !signature) {
          ...
          print.info('Usage: tempesttouch reputation did link --did <did> --public-key <key> --signature <sig>');
          break;
        }
        ...
        if (result.did) {
          ...
          print.info('No DID found. Run: tempesttouch reputation did claim');
        }
    ...
    case 'credentials': {
      ...
      if (!did) {
        ...
        if (!myDid?.did) {
          print.error('Usage: tempesttouch reputation credentials <did>');
          print.info('Or claim a DID first: tempesttouch reputation did claim');
          process.exit(1);
        }
    ...
    case 'receipts': {
      ...
      if (!did) {
        ...
        const myDid = await getMyDid(client);
        if (!myDid?.did) {
          print.error('Usage: tempesttouch reputation receipts <did>');
          print.info('Or claim a DID first: tempesttouch reputation did claim');
          process.exit(1);
        }
    ...
    case 'badge': {
      ...
      if (!did) {
        ...
        const myDid = await getMyDid(client);
        if (!myDid?.did) {
          print.error('Usage: tempesttouch reputation badge <did>');
          process.exit(1);
        }
        const { getBadgeUrl } = await import('../src/reputation.js');
        const url = getBadgeUrl(client.baseUrl || 'https://tempesttouch.com', myDid.did);
        ...
        const { getBadgeUrl } = await import('../src/reputation.js');
        const url = getBadgeUrl(client.baseUrl || 'https://tempesttouch.com', did);
        print.success('Badge URL:');
        print.info(`  ${url}`);
        print.info('');
        print.info('Markdown embed:');
  ...
  switch (subcommand) {
    case 'create': {
      ...
      if (!amount || amount <= 0) {
        ...
        print.info('Example: tempesttouch payout create --amount 5000 --description "Weekly payout"');
        process.exit(1);
      }
    ...
    case 'get': {
      const id = args[0];
      if (!id) {
        print.error('Usage: tempesttouch payout get <id>');
        process.exit(1);
      }
  ...
  switch (subcommand) {
    case 'create': {
      ...
      if (!businessId || !amount) {
        ...
        print.info('Example: tempesttouch card create --business-id biz_123 --amount 5000 --description "Order #123"');
        return;
      }
    ...
    case 'get': {
      ...
      if (!id) {
        print.error('Payment ID required');
        print.info('Usage: tempesttouch card get <id>');
        return;
      }
    ...
    case 'connect': {
      ...
      if (!connectSubcommand) {
        print.error('Usage: tempesttouch card connect <onboard|status> <merchantId>');
        return;
      }
      ...
      switch (connectSubcommand) {
        case 'onboard': {
          if (!merchantId) {
            ...
            print.info('Usage: tempesttouch card connect onboard <merchantId> --country <ISO-2> [--email <email>]');
        ...
        case 'status': {
          if (!merchantId) {
            print.error('Merchant ID required');
            print.info('Usage: tempesttouch card connect status <merchantId>');
            return;
          }
    ...
    case 'escrow': {
      ...
      if (!escrowSubcommand) {
        print.error('Usage: tempesttouch card escrow <release|refund> <id>');
        return;
      }
      ...
      switch (escrowSubcommand) {
        case 'release': {
          if (!escrowId) {
            ...
            print.info('Usage: tempesttouch card escrow release <id> [--reason "..."]');
            return;
          }
        ...
        case 'refund': {
          if (!escrowId) {
            print.error('Escrow ID required');
            print.info('Usage: tempesttouch card escrow refund <id> [--reason "..."] [--amount <cents>]');
            return;
          }
  ...
  switch (subcommand) {
    ...
    case 'address': {
      ...
      if (!username) {
        ...
        if (result.lightning_address) {
          ...
          console.log('No Lightning Address registered. Use: tempesttouch ln address --username <name>');
        }
      } else {
    ...
    case 'balance': {
      ...
      if (ln) {
        ...
        console.log('No Lightning balance. Enable Lightning first: tempesttouch ln enable');
      }
      break;
    }
  ...
  switch (subcommand) {
    ...
    case 'create': {
      ...
      if (!name || !redirectUri) {
        ...
        print.info('Example: tempesttouch oauth create --name "My App" --redirect-uri https://myapp.com/callback --scope openid,profile');
        return;
      }
    ...
    case 'get': {
      ...
      if (!id) {
        ...
        print.info('Usage: tempesttouch oauth get <client-id>');
        return;
      }
    ...
    case 'delete': {
      ...
      if (!id) {
        print.error('OAuth client ID required');
        print.info('Usage: tempesttouch oauth delete <client-id>');
        return;
      }
...
Usage: tempesttouch x402 <command> [options]
...
`);
  ...
  switch (subcommand) {
    case 'status': {
      ...
      const baseUrl = flags['base-url'] || config.baseUrl || 'https://tempesttouch.com';
      ...
      try {
        ...
        if (response.status === 401) {
          print.error('API key required or invalid. Set with: tempesttouch config set apiKey <key>');
    ...
    case 'test': {
      ...
      if (!url) {
        print.error('URL required: tempesttouch x402 test --url http://localhost:3000/api/premium');
        return;
      }
