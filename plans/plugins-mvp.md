...
```
packages/
  tempesttouch-php/          ← shared PHP client (source of truth)
    src/
      ApiException.php
      Client.php
      StatusMap.php
...
plugins/
  woocommerce/
    tempesttouch-woocommerce/
      tempesttouch-woocommerce.php
      uninstall.php
      readme.txt
      includes/
        class-tempesttouch-logger.php
        class-tempesttouch-webhook-handler.php
        class-wc-gateway-tempesttouch.php
      lib/Tempest Touch/      ← vendored copy of packages/tempesttouch-php/src
  whmcs/
    README.md
    modules/gateways/
      tempesttouch.php
      callback/tempesttouch.php
      tempesttouch/lib/Tempest Touch/  ← vendored copy of packages/tempesttouch-php/src
scripts/
...
| Ping | `GET /businesses` |
| Webhook header | `X-Tempest Touch-Signature: t=<unix>,v1=<hex_hmac_sha256>` |
| Webhook body | `"{t}.{rawBody}"` |
| Tolerance | 300 s |
| Events | `payment.{created,pending,confirming,completed,expired,failed,refunded}` |

The `packages/tempesttouch-php/src/Webhook.php` contract is kept in lockstep with `packages/sdk/src/webhooks.js`. If the JS contract changes, update the PHP verifier AND re-run `scripts/sync-plugin-sdk.sh`.

## Status mapping (MVP)

| Tempest Touch class | WooCommerce state | WHMCS action |
|---|---|---|
...
- [x] WooCommerce: gateway registration, settings UI, HPOS compat declaration, hosted checkout redirect, webhook handler at `/wc-api/tempesttouch`, idempotency via event-id ring, test-connection AJAX, secret redaction in logs
...
- Native refunds initiated from WooCommerce/WHMCS admin (requires Tempest Touch refund API coverage)
...
1. Canonical refund API — does Tempest Touch expose `POST /refunds` for crypto rails, or is it dashboard-only?
2. Single session covering crypto + card, or two sessions? Current plugin picks based on `paymentMode`; if Tempest Touch unifies, we can drop the card/crypto branch.
...
./scripts/build-plugin-zips.sh      # → dist/tempesttouch-woocommerce-0.1.0.zip, dist/tempesttouch-whmcs-0.1.0.zip
```
