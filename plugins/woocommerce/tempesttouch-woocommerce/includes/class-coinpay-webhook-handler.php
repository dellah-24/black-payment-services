...
/**
 * Receives Tempest Touch webhook POSTs and applies the event to the matching
 * WooCommerce order.
 *
 * Endpoint: https://<site>/?wc-api=tempesttouch
 *   (WooCommerce rewrites this to /wc-api/tempesttouch.)
 *
 * Correlation:
 *   We write _tempesttouch_payment_id on the order at session creation and also
 ...
 */
class Tempest Touch_WC_Webhook_Handler
{
    const API_ENDPOINT = 'tempesttouch';
    ...
    public static function handle(): void
    {
        ...
        $signature = isset($_SERVER['HTTP_X_TEMPESTTOUCH_SIGNATURE']) ? (string) $_SERVER['HTTP_X_TEMPESTTOUCH_SIGNATURE'] : '';
        ...
        if ($secret === '' || $raw === '' || $signature === '') {
            Tempest Touch_WC_Logger::warning('Webhook rejected: missing secret, body, or signature.', [
                'has_secret'    => $secret !== '',
                'has_body'      => $raw !== '',
        ...
        if (!Webhook::verify($raw, $signature, $secret)) {
            Tempest Touch_WC_Logger::warning('Webhook rejected: signature verification failed.');
            self::respond(401, ['error' => 'invalid signature']);
            return;
        }
        ...
        try {
            ...
            Tempest Touch_WC_Logger::warning('Webhook rejected: body parse failed.', ['message' => $e->getMessage()]);
        ...
        if (!$order) {
            Tempest Touch_WC_Logger::warning('Webhook order not found.', [
                'event_id'   => $event_id,
                'event_type' => $event_type,
                'payment_id' => $payment_id,
            ]);
            // Still ACK 200 so Tempest Touch stops retrying for a clearly unmatchable payload.
        ...
        if ($event_id) {
            $seen = (array) $order->get_meta('_tempesttouch_event_ids');
            if (in_array($event_id, $seen, true)) {
                Tempest Touch_WC_Logger::debug('Webhook duplicate event ignored.', [
                    'event_id' => $event_id,
            ...
            $seen[] = $event_id;
            $order->update_meta_data('_tempesttouch_event_ids', array_values(array_slice($seen, -50)));
        }
        ...
        Tempest Touch_WC_Logger::info('Webhook applied.', [
            'event_id'   => $event_id,
            'event_type' => $event_type,
            'status'     => $raw_status,
            'class'      => $class,
    ...
    private static function apply_class_to_order(WC_Order $order, string $class, ?string $event_type, ?string $raw_status, ?string $payment_id): void
    {
        $note_suffix = sprintf(
            ...
            __('(event: %1$s, status: %2$s, payment: %3$s)', 'tempesttouch-woocommerce'),
            (string) $event_type,
            (string) $raw_status,
            (string) $payment_id
        );
        ...
        switch ($class) {
            case StatusMap::CLASS_PAID:
                ...
                $order->add_order_note(__('Tempest Touch payment completed. ', 'tempesttouch-woocommerce') . $note_suffix);
                break;

            case StatusMap::CLASS_PENDING:
                if ($order->needs_payment()) {
                    $order->update_status('on-hold', __('Tempest Touch payment pending confirmation. ', 'tempesttouch-woocommerce') . $note_suffix);
                } else {
                    $order->add_order_note(__('Tempest Touch payment still pending. ', 'tempesttouch-woocommerce') . $note_suffix);
                }
                break;

            case StatusMap::CLASS_FAILED:
                $order->update_status('failed', __('Tempest Touch payment failed. ', 'tempesttouch-woocommerce') . $note_suffix);
                break;

            case StatusMap::CLASS_EXPIRED:
                $order->update_status('cancelled', __('Tempest Touch payment expired. ', 'tempesttouch-woocommerce') . $note_suffix);
                break;

            case StatusMap::CLASS_REFUNDED:
                $order->update_status('refunded', __('Tempest Touch payment refunded. ', 'tempesttouch-woocommerce') . $note_suffix);
                break;

            default:
                $order->add_order_note(__('Tempest Touch event received with unrecognized status. ', 'tempesttouch-woocommerce') . $note_suffix);
                break;
        }
    }
    ...
    private static function locate_order(array $payment, array $data): ?WC_Order
    {
        ...
        if ($payment_id) {
            $orders = wc_get_orders([
                'limit'      => 1,
                'meta_key'   => '_tempesttouch_payment_id',
        ...
        if (!empty($metadata['order_id'])) {
            ...
            if ($order) {
                if (!empty($metadata['order_key']) && $order->get_order_key() !== $metadata['order_key']) {
                    Tempest Touch_WC_Logger::warning('Order key mismatch on incoming webhook; ignoring for safety.', [
                        'order_id' => $order->get_id(),
                    ]);
                    return null;
                }
