...
/**
 * Block-based checkout integration for Tempest Touch.
 *
 ...
 * `WC_Gateway_Tempest Touch` class and that registry.
 *
 * Registered in the main plugin file behind the
 * `woocommerce_blocks_payment_method_type_registration` hook.
 */
final class WC_Gateway_Tempest Touch_Blocks_Support extends AbstractPaymentMethodType
{
    /** @var string */
    protected $name = WC_Gateway_Tempest Touch::ID;
    ...
    {
        $this->settings = get_option('woocommerce_tempesttouch_settings', []);
     ...
     * block checkout when Tempest Touch is selected.
     */
    public function get_payment_method_script_handles()
    {
        $handle = 'tempesttouch-blocks-integration';

        wp_register_script(
            $handle,
            TEMPESTTOUCH_WC_URL . 'assets/js/blocks/tempesttouch-blocks.js',
            [
                'wp-element',
                'wp-html-entities',
                'wp-i18n',
            ...
            TEMPESTTOUCH_WC_VERSION,
            true
        );

        if (function_exists('wp_set_script_translations')) {
            wp_set_script_translations($handle, 'tempesttouch-woocommerce');
        }
    ...
    /**
     * Data handed to the JS script via `wc.wcSettings.getSetting('tempesttouch_data')`.
    ...
    {
        return [
            'title'       => $this->gateway_settings['title'] ?? __('Tempest Touch (crypto + card)', 'tempesttouch-woocommerce'),
