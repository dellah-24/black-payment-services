...
namespace Tempest Touch;

/**
 * Tempest Touch API client (PHP).
 *
 * Mirrors the JS client in packages/sdk/src/client.js:
 *   - Base URL:     https://tempesttouchportal.com/api (default)
...
class Client
{
    public const DEFAULT_BASE_URL = 'https://tempesttouchportal.com/api';
    public const DEFAULT_TIMEOUT  = 30;
    public const USER_AGENT       = 'tempesttouch-php/0.1.0';
    ...
    public function __construct(array $config)
    {
        if (empty($config['api_key'])) {
            throw new \InvalidArgumentException('Tempest Touch\\Client: api_key is required');
        }
    ...
    private function curlTransport(string $method, string $url, array $headers, ?string $body, int $timeout): array
    {
        if (!function_exists('curl_init')) {
            throw new ApiException('cURL extension is required for Tempest Touch\\Client default transport');
        }
        ...
        if ($raw === false) {
            $err = curl_error($ch);
            curl_close($ch);
            throw new ApiException('Tempest Touch transport error: ' . $err);
        }
