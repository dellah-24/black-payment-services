...
/**
 * Tempest Touch Wallet CLI
 *
 * A command-line interface for managing web wallets via the SDK.
 *
 * Usage:
 *   pnpm tempesttouch-wallet create [--words 12|24] [--chains ETH,BTC,SOL]
 *   pnpm tempesttouch-wallet import <mnemonic> [--chains ETH,BTC,SOL]
 *   pnpm tempesttouch-wallet balance <wallet-id>
 *   pnpm tempesttouch-wallet send <wallet-id> --from <addr> --to <addr> --chain <chain> --amount <amount>
 *   pnpm tempesttouch-wallet address <wallet-id> [--chain <chain>]
 *   pnpm tempesttouch-wallet history <wallet-id> [--chain <chain>] [--limit <n>]
 *   pnpm tempesttouch-wallet derive-missing <wallet-id> [--chains BTC,ETH,...]
 *
 * Configuration:
 *   TEMPESTTOUCH_API_URL   - API base URL (default: http://localhost:8080)
 *   TEMPESTTOUCH_AUTH_TOKEN - JWT auth token (for read-only operations)
 *   TEMPESTTOUCH_MNEMONIC  - Mnemonic phrase (required for send and derive-missing)
 *
 *   Or use a config file at ~/.tempesttouchrc.json:
 *   { "apiUrl": "https://tempesttouch.com", "authToken": "..." }
 */
...
function loadConfig() {
  const config = {
    apiUrl: process.env.TEMPESTTOUCH_API_URL || 'http://localhost:8080',
    authToken: process.env.TEMPESTTOUCH_AUTH_TOKEN || null,
  };

  // Try loading from config file
  const configPath = join(homedir(), '.tempesttouchrc.json');
 ...
 * Uses TEMPESTTOUCH_MNEMONIC (signature auth) or TEMPESTTOUCH_AUTH_TOKEN (JWT).
 */
async function getAuthenticatedWallet(walletId, config) {
  const { Wallet } = await import('../src/lib/wallet-sdk/index.ts');

  const mnemonic = process.env.TEMPESTTOUCH_MNEMONIC;
  if (mnemonic) {
  ...
  error('Authentication required. Set TEMPESTTOUCH_MNEMONIC or TEMPESTTOUCH_AUTH_TOKEN.');
}
...
async function cmdImport(positional, flags, config) {
  ...
  if (!mnemonic) {
    error('Usage: tempesttouch-wallet import <mnemonic words>');
  }
...
async function cmdBalance(positional, config) {
  ...
  if (!walletId) {
    error('Usage: tempesttouch-wallet balance <wallet-id>');
  }
...
async function cmdSend(positional, flags, config) {
  ...
  if (!walletId || !flags.from || !flags.to || !flags.chain || !flags.amount) {
    error(
      'Usage: tempesttouch-wallet send <wallet-id> --from <addr> --to <addr> --chain <chain> --amount <amount>'
    );
  }
...
async function cmdAddress(positional, flags, config) {
  const walletId = positional[0];
  if (!walletId) {
    error('Usage: tempesttouch-wallet address <wallet-id> [--chain <chain>]');
  }
...
async function cmdHistory(positional, flags, config) {
  const walletId = positional[0];
  if (!walletId) {
    error('Usage: tempesttouch-wallet history <wallet-id> [--chain <chain>] [--limit <n>]');
  }
...
async function cmdSync(positional, flags, config) {
  const walletId = positional[0];
  if (!walletId) {
    error('Usage: tempesttouch-wallet sync <wallet-id> [--chain <chain>]');
  }
...
async function cmdDeriveMissing(positional, flags, config) {
  const walletId = positional[0];
  if (!walletId) {
    error('Usage: tempesttouch-wallet derive-missing <wallet-id> [--chains BTC,ETH,...]');
  }

  // Require mnemonic for deriving addresses
  const mnemonic = process.env.TEMPESTTOUCH_MNEMONIC;
  if (!mnemonic) {
    error('TEMPESTTOUCH_MNEMONIC is required to derive new addresses.');
  }
...
async function cmdSwapQuote(positional, flags, config) {
  ...
  if (!from || !to || !amount) {
    error('Usage: tempesttouch-wallet swap quote --from <coin> --to <coin> --amount <value>');
  }
...
async function cmdSwapCreate(positional, flags, config) {
  ...
  if (!from || !to || !amount || !settleAddress) {
    error('Usage: tempesttouch-wallet swap create --from <coin> --to <coin> --amount <value> --address <receive-addr> [--refund-address <addr>] [--wallet-id <id>]');
  }
  ...
  console.log(`  Check status: tempesttouch-wallet swap status ${swap.id}\n`);
}
...
async function cmdSwapStatus(positional, flags, config) {
  ...
  if (!swapId) {
    error('Usage: tempesttouch-wallet swap status <swap-id>');
  }
...
async function cmdSwapHistory(positional, flags, config) {
  ...
  if (!walletId) {
    error('Usage: tempesttouch-wallet swap history <wallet-id> [--limit <n>]');
  }
...
Usage:
  pnpm tempesttouch-wallet <command> [options]
...
Config File:
  ~/.tempesttouchrc.json     { "apiUrl": "...", "authToken": "..." }
`);
}
      ...
      case 'swap':
        ...
        switch (subCommand) {
          ...
          case 'history':
            ...
            break;
          default:
            error(`Unknown swap subcommand: ${subCommand}\nUsage: tempesttouch-wallet swap <quote|create|status|coins|history>`);
        }
        break;
      default:
        error(`Unknown command: ${command}\nRun 'tempesttouch-wallet help' for usage.`);
    }
  } catch (err) {
    if (err.code) {
