...
You are integrating Tempest Touch's create-payment endpoint with **both** rails enabled (crypto and credit card). The customer either picks crypto on the Tempest Touch-hosted page or completes a Stripe Checkout for card — your code handles both via one merchant webhook.
...
TEMPESTTOUCH_MERCHANT_ID=<your business id, UUID>
...
- `TEMPESTTOUCH_MERCHANT_ID` — the business UUID from the URL on that same page.
- `TEMPESTTOUCH_WEBHOOK_SECRET` — same business page → **Webhooks** tab → create endpoint → **Signing Secret**. Each endpoint has its own secret.
- **Card support also requires Stripe Connect.** Open the business page → **Stripe** tab → connect. You don't add any Stripe env vars on your side — Tempest Touch's platform routes the card flow through its own connected-account Stripe Checkout.

## Endpoint and required body

`POST {TEMPESTTOUCH_API_URL}/api/payments/create` (the documented `/api/payments/unified` does not exist — use this endpoint).

```jsonc
{
  "business_id": "<TEMPESTTOUCH_MERCHANT_ID>",
  "amount_usd": 49.99,
...
- `currency` — even when you only care about the card, Tempest Touch requires a crypto currency on the request. Default to `usdc_pol`.
- `success_url` / `cancel_url` — used **verbatim** as Stripe Checkout's success/cancel URLs. **If you omit them, Stripe will redirect the customer to `tempesttouch.com/pay/<id>?status=success` after paying** (a Tempest Touch-hosted landing page that doesn't bounce back to your domain). The `redirect_url` field is documented for crypto only and is ignored on the card leg.
- `redirect_url` — used by the Tempest Touch-hosted page after a crypto payment completes (5-second auto-redirect). Always include both `success_url` and `redirect_url`.
...
- **Crypto:** render the `payment_address` + `amount_crypto` (or redirect to `https://tempesttouch.com/pay/<id>` for the hosted page).

## Webhook

You receive **one** event per completed payment regardless of rail:

- Card → `payment.confirmed` (Stripe → Tempest Touch → you)
- Crypto → `payment.forwarded` (chain → Tempest Touch → you, includes on-chain `tx_hash`)
...
- Always pass `success_url` AND `cancel_url` — without them the card flow strands users on tempesttouch.com.
