...
X402_FACILITATOR_URL=https://tempesttouch.com/api/x402
X402_PRICE_USDC=0.01
```

Where to find them:
- `X402_PAY_TO` — your own merchant wallet address. For crypto wallets you control already, paste it directly. For one managed by Tempest Touch, copy it from `https://tempesttouch.com/settings/wallets` (the chain you intend to receive on).
- `X402_FACILITATOR_URL` — always `https://tempesttouch.com/api/x402` for production. Tempest Touch is the multi-chain x402 facilitator; no key needed.
...
1. **Install the middleware** (Node example):
   ...
   ```bash
   npm install @tempesttouch/x402
   ```

2. **Wrap the route:**

   ```js
   import { createX402Middleware, buildPaymentRequired, verifyX402Payment } from '@tempesttouch/x402';

   const x402 = createX402Middleware({
     price: { amount: '0.01', currency: 'USDC', chain: 'base' },
     payTo: '0xYourMerchantWalletAddress',
     facilitator: 'https://tempesttouch.com/api/x402',
   });
